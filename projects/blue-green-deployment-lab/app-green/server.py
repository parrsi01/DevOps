...
class Handler(BaseHTTPRequestHandler):
    server_version = "BlueGreenLab/2.0"
    ...
    def do_GET(self):
        ...
        if parsed.path == "/":
            ...
            if not schema_compatible(state):
                return self._json(500, {"error": "schema_incompatible", "version": APP_VERSION})
        ...
        if parsed.path == "/state":
            return self._json(200, {"version": APP_VERSION, "state": read_state()})
        ...
        # green can migrate schema to v2 and also downgrade for lab recovery exercises
        if parsed.path == "/control/migrate":
            q = parse_qs(parsed.query)
            target = int(q.get("schema", ["2"])[0])
...
if __name__ == "__main__":
    ensure_state()
    if os.getenv("AUTO_MIGRATE_TO_V2", "0") == "1":
        st = read_state()
        st["schema_version"] = 2
        st["last_writer"] = APP_VERSION
        write_state(st)
